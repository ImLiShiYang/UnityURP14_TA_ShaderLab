...
// Footprint Local RT
// ============================================================
// 这些参数由 FootprintLocalRTPainter.cs 通过 Shader.SetGlobalXXX 传入。
//
// _FootprintRT 通道约定：
// R = 脚印 mask
// G = 脚印 depth / AO
...
TEXTURE2D(_FootprintRT);
SAMPLER(sampler_FootprintRT);
...
// 先写死参数，跑通后再做成材质 Inspector 参数。
float _FootprintAOMin = 0.35;
float _FootprintAOStrength = 1.5;

// 根据世界坐标计算脚印 RT 的 UV。
// Terrain 上每个像素都有自己的 worldPos。
// worldPos.xz 会被映射到 _FootprintRect 对应的 0~1 范围。
...
// 判断脚印 UV 是否超出当前局部 RT 范围。
bool IsFootprintUVOutside01(float2 uv)
{
    return uv.x < 0.0 || uv.x > 1.0 || uv.y < 0.0 || uv.y > 1.0;
}

// 安全采样脚印 RT。
// 如果脚印系统没启用，或者当前像素不在局部 RT 范围内，就返回 0。
half4 SampleFootprintRTData(float2 footprintUV)
{
    if (_FootprintEnabled < 0.5)
        return half4(0, 0, 0, 0);

    if (IsFootprintUVOutside01(footprintUV))
        return half4(0, 0, 0, 0);
...
#endif
{
    ...
    // ============================================================
    // Footprint RT AO Blend
    // ============================================================
    // 这里是第一版脚印接入点。
    //
    // inputData.positionWS 是当前 Terrain 像素的世界坐标。
    // GetFootprintUV 会把 worldPos.xz 映射到 FootprintLocalRTPainter 当前维护的局部 RT 范围。
    //
    // _FootprintRT:
    // R = 脚印 mask
    // G = 脚印 depth / AO
    //
    // 这一版先只用 G 通道压暗地面颜色。
    // 好处是最稳定，不会破坏 Terrain 原本的法线、阴影、Splat 混合。
    {
        float2 footprintUV = GetFootprintUV(inputData.positionWS);
        half4 footprint = SampleFootprintRTData(footprintUV);
        ...
        // _FootprintAOMin = 最暗能压到多少。
        // 0.35 表示最深处保留 35% 亮度。
        half footprintAO = lerp(1.0h, (half)_FootprintAOMin, aoMask);

        // 只压暗 albedo。
        // 这样原本 Terrain/Lit 的光照、阴影、法线、SplatMap 都保留。
        albedo *= footprintAO;

        // 可选：顺便稍微压 occlusion，让脚印更像 AO。
        // 如果你觉得脚印太黑或影响过重，可以注释掉这一行。
        occlusion *= footprintAO;
    }
