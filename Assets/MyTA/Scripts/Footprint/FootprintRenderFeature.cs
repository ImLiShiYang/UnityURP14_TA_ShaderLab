...
/// <summary>
/// 脚印渲染用的 URP RenderFeature。
///
/// 这个 RenderFeature 的职责：
///
/// 1. 只在 FootstepCamera 上执行。
///    因为 FootstepCamera 是专门用来从上往下拍摄脚印 Brush 的相机。
///
/// 2. 每帧先清空 CurrentBrushRT。
///    CurrentBrushRT 是“当前帧临时脚印图”，不是历史图。
///    所以它必须每帧清成默认法线 + alpha 0。
///
/// 3. 只绘制 FootprintBrush Layer。
///    也就是只把脚印 Brush prefab 画进 CurrentBrushRT。
///
...
/// A   = 脚印 mask / depression，默认值是 0
/// </summary>
public class FootprintRenderFeature : ScriptableRendererFeature
{
    /// <summary>
    /// 控制要绘制哪些渲染队列。
    ///
    ...
    /// Transparent：
    ///     只绘制透明物体。
    ///     你的 FootprintBrush shader 使用的是 Queue = Transparent，
    ///     所以一般应该选 Transparent。
    ///
    /// All：
    ///     不区分队列，全部都尝试绘制。
    ...
    public class Settings
    {
        ...
        /// </summary>
        public string profilerTag = "Footprint Render Pass";

        /// <summary>
        /// RenderPass 执行时机。
        ///
        /// 建议使用 AfterRendering：
        /// - FootstepCamera 自己可能有普通渲染 / FinalBlit。
        ...
        ///
        /// 你的脚印 prefab 必须放到这个 Layer。
        /// 地面、玩家、其他物体不能在这个 Layer 上，
        /// 否则它们也可能被画进 CurrentBrushRT。
        /// </summary>
        [Tooltip("只绘制 FootprintBrush Layer。")]
        public LayerMask footprintLayerMask;

        /// <summary>
        /// Brush 材质所在的渲染队列。
        ///
        /// 你的 Footprints/URP_FootprintBrush_NormalHeightSeparate
        ...
        /// <summary>
        /// 是否只允许 FootstepCamera 执行这个 Pass。
        ///
        /// 必须开启。
        /// 否则主相机也会执行这个 RenderFeature，
        /// 导致 CurrentBrushRT 被错误清空或错误写入。
        /// </summary>
        [Tooltip("是否只在 FootstepCamera 上执行。")]
    ...
    ///
    /// 在这里创建一次 FootprintPass，
    /// 后面每帧只需要把它 Enqueue 到 Renderer 里。
    /// </summary>
    public override void Create()
    {
        pass = new FootprintPass(settings);
    ...
    ///
    /// 这里要做很多保护判断：
    /// 1. FootprintRTManager 是否存在。
    /// 2. 三张 RT 是否已经创建。
    /// 3. 累积材质是否存在。
    /// 4. 当前相机是不是 FootstepCamera。
    ///
    /// 全部满足后，才把 FootprintPass 加入渲染队列。
    /// </summary>
    public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
    {
        // FootprintRTManager.Active 是当前场景里的脚印 RT 管理器。
        // RenderFeature 通过它拿 CurrentBrushRT / AccumA / AccumB / 材质等数据。
        FootprintRTManager manager = FootprintRTManager.Active;
    ...
    private class FootprintPass : ScriptableRenderPass
    {
        ...
        ///
        /// 你的 FootprintBrush shader 如果没有写 LightMode，
        ...
        /// 当前脚印系统的 RT 管理器。
        /// </summary>
        private FootprintRTManager manager;
        ...
        /// 2. LayerMask：只绘制 FootprintBrush Layer
        /// </summary>
        ...
        /// 构造函数。
        /// </summary>
        public FootprintPass(Settings settings)
        {
        ...
        /// </summary>
        public void Setup(FootprintRTManager manager, Settings settings)
        {
        ...
        ///
        /// 1. 清空 CurrentBrushRT。
        /// 2. 绘制 FootprintBrush Layer 到 CurrentBrushRT。
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            // CommandBuffer 用于记录 GPU 命令。
            // 这里所有 SetRenderTarget / Clear / Blit 都通过 cmd 执行。
            ...
            // 根据 QueueMode 决定排序方式。
            // Transparent 通常用 CommonTransparent。
            // Opaque 通常用 CommonOpaque。
            ...
            // 真正绘制场景中符合条件的 Renderer。
            //
            // 条件来自 filteringSettings：
            // 1. 必须在 footprintLayerMask 上
            // 2. 必须在指定 RenderQueueRange 里
            //
            // 绘制结果会写入当前 RenderTarget，也就是 CurrentBrushRT。
            ...
            // =====================================================
            // 3. 累积 CurrentBrushRT + AccumA -> AccumB
            // =====================================================
            //
            // 不是每帧都应该累积。
            //
            // 只有这些情况才需要：
            // 1. 有新的 Brush 被生成
            // 2. FootstepCamera 中心移动，需要滚动历史 RT
            // 3. reduceVal > 0，需要让历史脚印淡出
            //
            // 如果人物站着不动，并且没有新脚印，也不需要淡出，
            // 就跳过累积，避免同一个脚印被重复叠加。
